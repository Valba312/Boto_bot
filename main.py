...
def cmd_newtask(m):
    ...
    # ждем текст
    user_states[uid] = 'await_text'
    bot.send_message(cid, "Введите текст задачи:", message_thread_id=tid)

@bot.message_handler(func=lambda m: user_states.get(m.from_user.id)=='await_text',
                     content_types=['text'])
def handle_newtask_text(m):
    ...
    try:
        ...
        logger.exception("Ошибка БД при сохранении новой задачи")
        try:
            bot.delete_message(cid, mid)
        except ApiTelegramException:
            pass
        bot.reply_to(m, "❗ Не удалось сохранить задачу в базу.")
    finally:
        user_states.pop(uid, None)
...
def cb_accept(cb):
    ...
    try:
        ...
        return bot.answer_callback_query(cb.id, text="Ошибка формата данных", show_alert=True)
    ...
    try:
        ...
        return bot.send_message(cid, "❗ Не удалось прочесть задачу.", message_thread_id=tid)
    ...
    try:
        bot.edit_message_text(new_md, cid, mid, parse_mode='Markdown')
    except ApiTelegramException as e:
        logger.exception("Ошибка API при редактировании задачи в «Принято»")
        bot.send_message(cid, "❗ Не удалось обновить статус.", message_thread_id=tid)
        return

    try:
        db.update_task_status(cid, tid, mid, 'принято', taker)
    except DatabaseError:
        logger.exception("Ошибка БД при сохранении статуса «принято»")
        bot.send_message(cid, "❗ Не удалось записать статус в БД.", message_thread_id=tid)
        return

    bot.answer_callback_query(cb.id, text="✅ Помечено как принято", show_alert=False)
